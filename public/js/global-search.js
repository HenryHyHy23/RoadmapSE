document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateBadges() {
    ...
    if (searchFilters.subjectCode) {
      badgeContainer.innerHTML += `
        <span class="badge d-flex align-items-center shadow-sm my-auto" 
              style="background-color: #f97316; color: white; font-size: 0.9rem; padding: 7px 14px; border-radius: 50px; line-height: 1; gap: 8px;">
            <span class="text-truncate">${searchFilters.subjectCode}</span>
            <i class="bi bi-x-circle-fill text-white flex-shrink-0" style="cursor:pointer; opacity: 0.8; transition: 0.2s; font-size: 1.05rem;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'" onclick="removeFilter('subject')"></i>
        </span>`;
    }
    ...
    if (searchFilters.lessonName) {
      badgeContainer.innerHTML += `
        <span class="badge d-flex align-items-center shadow-sm my-auto" 
              style="background-color: #0ea5e9; color: white; font-size: 0.9rem; padding: 7px 14px; border-radius: 50px; line-height: 1; max-width: 50vw; gap: 8px;">
            <span class="text-truncate">${searchFilters.lessonName}</span> 
            <i class="bi bi-x-circle-fill text-white flex-shrink-0" style="cursor:pointer; opacity: 0.8; transition: 0.2s; font-size: 1.05rem;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'" onclick="removeFilter('lesson')"></i>
        </span>`;
    }
  }

  function renderSuggestions() {
    // 1. DỌN SẠCH CLASS CŨ, ÉP CHUẨN MỚI: Cấm thẻ này nằm ngang, bắt buộc rớt dòng và chiếm 100% độ rộng
    suggestionContainer.className =
      "d-flex flex-wrap align-items-center gap-2 mt-2 w-100";
    suggestionContainer.innerHTML = "";

    if (!searchFilters.subjectCode) {
      suggestionContainer.innerHTML += `<span class="text-muted fw-bold small flex-shrink-0" style="white-space: nowrap;">Chọn môn:</span>`;
      ...
      suggestionContainer.innerHTML += `<span class="text-muted fw-bold small flex-shrink-0" style="white-space: nowrap;">Chọn chủ đề:</span>`;
        ...
        activeSubject.subLessons.forEach((group) => {
          if (group.subLessons) {
            group.subLessons.forEach((lesson) => {
              suggestionContainer.innerHTML += `
                <span class="badge rounded-pill border px-3 py-2 shadow-sm text-truncate" 
                      style="max-width: 100%; background-color: #ffffff; color: #64748b; border-color: #e2e8f0; cursor: pointer; font-size: 0.85rem; transition: all 0.2s;"
      ...
      suggestionContainer.innerHTML += `<span class="badge bg-success bg-opacity-10 text-success px-3 py-2 rounded-pill border border-success flex-grow-1" style="white-space: normal; text-align: left;">
          <i class="bi bi-check-circle-fill me-1"></i> Đã khoanh vùng. Hãy gõ từ khóa vào ô bên trên!
      </span>`;
    }
  }
